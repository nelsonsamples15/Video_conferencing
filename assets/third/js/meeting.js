...
var Meeting = function (socketioHost) { 
    ...
    var _pcConfig = {'iceServers': [{'urls': 'turn:asia.myturnserver.net', 'username':'allie@oopcode.com', 'credential':'topsecretpassword'}]};
                ...
                _offerChannels[partID].on('message', function (msg){
                    if (msg.dest===_myID) {
                        if (msg.type === 'answer') {
                            ...
                            console.log('got ice candidate fromss '+msg.from);
    ...
    function handleUserMedia(stream) {
        ...
        _localStream = stream;
        _defaultChannel.emit('message', {type:'newparticipant', from: _myID, room:_room});
    }
    ...
    function handleRemoteStreamAdded(from) {
	    return function(event) {
			...
			addRemoteVideo(event.streams[0], from);
