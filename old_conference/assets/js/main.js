...
// var serverIP = "http://t2.dev.codev-jp.com:8080";
...
btnVideoStart.onclick = function(e) {
    ...
    if (room == undefined || room.length <= 0) {}else{
        $(this).hide();
    }
}
...
function connect(room) {
    ...
    navigator.getUserMedia({
        ...
        $("#local-video").css({"height": "200", "width": "100%"});
        $(".local-video").addClass("add-new-back");
        $("#local-video-label").show();
...
function establishRTCConnection(room) {
    ...
    signallingServer.onGuestJoined = function(thisClient) {
        trace('guest joined!');
        console.log('try');
        // set local description and send to remote
        localPeerConnection[thisClient].createOffer(handleCreateOffer, handleCreateOfferError);
    }
    ...
    signallingServer.onReceiveSdp = function(sdp) {
        ...
        trace(localStream);
        $("#remote-video").css({"height": "200", "width": "100%"});
        $(".remote-video").addClass("add-new-back");
        $("#remote-video-label").show();
        ...
        if (localIsCaller) {
            ...
            localPeerConnection.setRemoteDescription(new RTCSessionDescription(
                sdp), setRemoteDescriptionSuccess, setRemoteDescriptionError);
        }
    ...
    function handleCreateOffer(sessionDescription){
            ...
            localPeerConnection[thisClient].setLocalDescription(sessionDescription);
    ...
    function addIceCandidateError(error) {
        console.log('addIceCandidateError error: ', error);
    }
    ...
    signallingServer.onReceiveICECandidate = function(candidate) {
        ...
        localPeerConnection.addIceCandidate(new RTCIceCandidate(candidate), addIceCandidateSuccess, addIceCandidateError);
    }
    ...
    // }
    // localPeerConnection.ontrack = function(data) {
    //     remoteVideo.src = window.URL.createObjectURL(data.streams[0]);
    // }


     localPeerConnection.onaddstream = function(data) {
        remoteVideo.src = window.URL.createObjectURL(data.stream);
        // var the_remote = '<video width="100%" height="200" autoplay="true" src="'+window.URL.createObjectURL(data.stream)+'" style="border-radius: 20px"></video>';
        
        // $(".remote-video").append(the_remote);
    }
